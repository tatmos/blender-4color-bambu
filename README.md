...
3. スクリプトを実行する。
   ...
   - またはテキストエディタに貼り付けて実行
4. **保存先ダイアログ**が開くので、出力ファイル（`.obj` または `.3mf`）のパスを指定する。拡張子で形式が決まります（.obj → 頂点色付き OBJ、.3mf → 3MF）。ダイアログを使わずスクリプトから直接 `process_scene()` を呼ぶ場合は、従来どおりスクリプト先頭の `OUTPUT_PATH` が使われます。
...
| `EXPORT_FORMAT` | ダイアログを使わず `process_scene()` を直接呼ぶときの形式。ダイアログで保存する場合はファイルの拡張子（.obj / .3mf）で自動判定される。 |
